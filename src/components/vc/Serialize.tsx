import {DidDocument} from "@/lib/verificationMaterialBuilder";

export default function Serialize({ vc, type }: { vc: string, type: 'jwt' | 'proof' }) {
    let result = vc;
    let validDocument: boolean;
    if (type === 'proof') {
      try {
        result = JSON.stringify(JSON.parse(vc), null, 2);
        validDocument = true;
      } catch (e) {
        // @ts-ignore
        result = e.toString();
